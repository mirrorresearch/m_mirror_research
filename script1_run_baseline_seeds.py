...
def parse_args():
    p = argparse.ArgumentParser(description="Run baseline model with multiple seeds and export per-seed metrics.")
...
def main():
    ...
    for seed in args.seeds:
        ...
        s = r["eval2022"]
        rows.append({"model": "baseline", "seed": seed, **s})
        print(f"AUC={s['auc']:.4f}, F1={s['f1_pos']:.4f}, th={s['threshold']:.4f}")

    with open(os.path.join(args.out_dir, "baseline_runs.json"), "w", encoding="utf-8") as f:
        json.dump(all_results, f, indent=2, ensure_ascii=False)

    pd.DataFrame(rows).to_csv(os.path.join(args.out_dir, "metrics_per_seed_baseline.csv"), index=False)
